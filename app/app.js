...
const state={cache:{},searchEngine:null,quiz:null,currentScope:{},currentExam:[],wrongBook:[],stats:{done:0,correct:0},relationGraph:null,lastEntityId:null,currentEntityPage:null,entityDisplayLimit:30,suppressSearchDefaultOnce:false,nav:{back:[],forward:[],current:{kind:'tab',tab:'guide'},restoring:false}};
const DEFAULT_QUERY='桂枝湯';
const DEFAULT_ENTITY_ID='can_2849adeebb';
function parseLimitValue(v,fallback=30){return String(v)==='all'?Infinity:Math.max(1,Number(v)||fallback)}
function limitLabel(n){return Number.isFinite(n)?String(n):'全部'}
function scheduleIdle(fn,timeout=500){if('requestIdleCallback' in window){window.requestIdleCallback(fn,{timeout});}else{setTimeout(fn,Math.min(timeout,180));}}
const titles={
...
function switchTab(tab){ $$('.tab').forEach(x=>x.classList.remove('active')); const el=$('#tab-'+tab); if(el)el.classList.add('active'); $$('#navTabs button').forEach(b=>b.classList.toggle('active',b.dataset.tab===tab)); $('#pageTitle').textContent=titles[tab]?.[0]||tab; $('#pageSub').textContent=titles[tab]?.[1]||''; const searchInit=()=>{const auto=!state.suppressSearchDefaultOnce;state.suppressSearchDefaultOnce=false;return initSearch({autoDefault:auto});}; const init={guide:renderGuidePage,search:searchInit,learning:initLearning,entity:initEntityDefault,tree:initTree,relationGraph:initRelationGraph,quiz:initQuiz,qhotspot:initQuestionHotspot,pivot:initPivot,formulaTools:initFormulaTools,shanghan:()=>initSourcePage('shanghan'),jingui:()=>initSourcePage('jingui'),wenbing:()=>initSourcePage('wenbing'),zhengzhi:initZhengzhi,diagnosis:initDiagnosis,health:renderHealth}[tab]; if(init)init().catch(e=>console.error('[Clinical1] 模組載入失敗:',tab,e));}
...
async function initSearch(opts={autoDefault:true}){
  const root=$('#searchRoot');
  if(!root)return;
  root.innerHTML=`<div class="card"><div class="search-row"><input id="searchInput" value="${esc(DEFAULT_QUERY)}" placeholder="搜尋：桂枝湯、氣分兼表、咳嗽 痰熱鬱肺、舌紅少苔"><select id="searchType"><option value="">全部類型</option><option value="entity">節點</option><option value="section">章節</option><option value="question">題目</option><option value="clause">條文</option><option value="structured_row">表格列</option></select><button class="btn primary" id="searchBtn">搜尋</button></div><div class="search-toolbar-2"><label>排序<select id="searchSort"><option value="score">相關度高到低</option><option value="type">依類型分組</option><option value="title">名稱筆畫／字序</option></select></label><label>顯示<select id="searchLayout"><option value="cards">區塊卡片</option><option value="list">條列清單</option><option value="compact">緊湊模式</option></select></label><label>筆數<select id="searchLimit"><option value="30" selected>30</option><option value="60">60</option><option value="100">100</option><option value="160">160</option><option value="300">300</option><option value="all">全部</option></select></label><span id="searchStatus" class="muted">預設查詢「桂枝湯」，可調整筆數後重新搜尋。</span></div></div><div id="searchResults" class="search-results cards"><div class="card light-placeholder"><h3>桂枝湯搜尋準備中</h3><p class="muted">開啟搜尋頁後會自動載入前 30 筆結果；可改成 60、100、300 或全部。</p></div></div>`;
  async function ensure(){if(state.searchEngine)return; $('#searchStatus').textContent='載入搜尋資料…'; const p=await loadJSON('data/search/search_payload_v3.json'); state.searchEngine=new Clinical1SearchEngineV3(p); $('#searchStatus').textContent=`已載入 ${p.docs.length.toLocaleString()} 筆搜尋文件。`;}
  async function run(){
    await ensure();
    const q=$('#searchInput').value.trim()||DEFAULT_QUERY;
    $('#searchInput').value=q;
    const typ=$('#searchType').value;
    const displayLimit=parseLimitValue($('#searchLimit').value,30);
    $('#searchStatus').textContent='搜尋中…';
    let allHits=state.searchEngine.search(q,{limit:Infinity,doc_type:typ||undefined});
    const sort=$('#searchSort').value;
    if(sort==='title')allHits.sort((a,b)=>String(a.title||'').localeCompare(String(b.title||''),'zh-Hant'));
    if(sort==='type')allHits.sort((a,b)=>String(a.doc_type||'').localeCompare(String(b.doc_type||''))||Number(b.score||0)-Number(a.score||0));
    const hits=Number.isFinite(displayLimit)?allHits.slice(0,displayLimit):allHits;
    renderSearchResults(hits,q,$('#searchLayout').value,allHits.length,displayLimit);
    $('#searchStatus').textContent=`顯示 ${hits.length.toLocaleString()} / ${allHits.length.toLocaleString()} 筆。`;
  }
  ['searchSort','searchLayout','searchType','searchLimit'].forEach(id=>$('#'+id).addEventListener('change',()=>{ if(state.searchEngine||$('#searchInput').value.trim()) run(); }));
  $('#searchBtn').onclick=run;
  $('#searchInput').addEventListener('keydown',e=>{if(e.key==='Enter')run()});
  if(opts.autoDefault){scheduleIdle(()=>{ if(document.querySelector('#tab-search.active')) $('#searchBtn')?.click(); },420);}
}
function renderSearchResults(hits,q,layout='cards',total=hits.length,displayLimit=30){
  const root=$('#searchResults');
  root.className='search-results '+layout;
  const group={entity:[],section:[],question:[],clause:[],structured_row:[],other:[]};
  hits.forEach(h=>(group[h.doc_type]||group.other).push(h));
  const labels={entity:'節點',section:'章節',question:'題目',clause:'條文',structured_row:'結構表格列',other:'其他'};
  const shownText=Number.isFinite(displayLimit)?`顯示 ${hits.length} / ${total} 筆`:`顯示全部 ${hits.length} 筆`;
  const body=Object.entries(group).filter(([k,v])=>v.length).map(([k,arr])=>`<h3 class="group-title">${labels[k]} (${arr.length})</h3>${arr.map(renderResult).join('')}`).join('');
  root.innerHTML=`<div class="muted result-count">查詢「${esc(q||'')}」${shownText}；可切換排序、顯示密度與筆數。</div>`+(body||'<div class="card">沒有結果。</div>');
}
...
async function initEntityDefault(){
  const root=$('#entityRoot');
  if(!root || root.dataset.loadingDefault==='1' || root.dataset.ready==='1')return;
  root.dataset.loadingDefault='1';
  try{await openEntity(DEFAULT_ENTITY_ID,false);}finally{root.dataset.loadingDefault='0';}
}
async function openEntity(id, record=true){
  state.lastEntityId=id;
  if(state.relationGraph&&state.relationGraph.state){state.relationGraph.state.selectedEntity=id;}
  if(record)commitSnapshot(makeSnapshot('entity',{id}));
  switchTab('entity');
  const root=$('#entityRoot');
  root.dataset.ready='1';
  root.innerHTML='<div class="card light-placeholder"><h3>載入節點頁…</h3><p class="muted">預設節點為桂枝湯；資料卡先顯示前 30 筆，可切換為全部。</p></div>';
  const p=await loadEntityPageL11(id);
  if(!p){root.innerHTML=`<div class="card"><h3>找不到節點</h3><p class="muted">節點 id：${esc(id)}</p><p>資料庫尚未建立可開啟的節點頁；可改用全庫搜尋追資料。</p><button class="btn primary" data-run-search="${esc(id)}">搜尋此 id</button></div>`;return}
  const ft=await loadJSON('data/formula_tools_payload_v3_1.json').catch(()=>({formula_blocks:{},zhengzhi_rows:[],wenbing_rows:[]}));
  const fblocks=ft.formula_blocks||await loadJSON('data/formula/formula_page_blocks_v3.json').catch(()=>({}));
  state.currentEntityPage={p,fb:fblocks[id],ft};
  root.innerHTML=renderEntityPage(p,fblocks[id],ft);
  bindEntityDisplayLimit();
}
function entityLimitOptions(){const v=limitLabel(state.entityDisplayLimit);return ['30','60','100','160','300','all'].map(x=>`<option value="${x}" ${v===x||(v==='全部'&&x==='all')?'selected':''}>${x==='all'?'全部':x}</option>`).join('')}
function takeByEntityLimit(arr){const lim=state.entityDisplayLimit||30; return Number.isFinite(lim)?(arr||[]).slice(0,lim):(arr||[])}
function entityLimitSummary(label,shown,total){return `<span class="muted">${esc(label)}：${shown} / ${total}</span>`}
function bindEntityDisplayLimit(){const sel=$('#entityDisplayLimit'); if(!sel)return; sel.value=limitLabel(state.entityDisplayLimit)==='全部'?'all':limitLabel(state.entityDisplayLimit); sel.onchange=()=>{state.entityDisplayLimit=parseLimitValue(sel.value,30); const cur=state.currentEntityPage; if(cur){$('#entityRoot').innerHTML=renderEntityPage(cur.p,cur.fb,cur.ft); bindEntityDisplayLimit();}};}
function renderEntityPage(p,fb,ft={}){
  const qstat=p.question_location_counts||{};
  const qsample=p.questions_sample||[];
  const sections=p.sections_sample||[];
  const out=p.relations_out_sample||[], inn=p.relations_in_sample||[];
  const ext=p.fused_v3_extension_refs||{};
  const qShown=takeByEntityLimit(qsample), secShown=takeByEntityLimit(sections), outShown=takeByEntityLimit(out), inShown=takeByEntityLimit(inn);
  return `<div class="entity-head"><div><h2 class="entity-title">${esc(p.name)}</h2><div><span class="badge">${esc(p.type)}</span>${(p.aliases||[]).slice(0,10).map(a=>`<span class="chip">${esc(a)}</span>`).join('')}</div></div><div class="entity-actions"><button class="btn primary" data-quiz-scope='${esc(JSON.stringify({entity_id:p.canonical_id,layer:'official_exam'}))}'>以此節點出題</button><button class="btn" data-tree-entity="${esc(p.canonical_id)}">看中心網路</button><button class="btn" data-hotspot-test="${esc(p.canonical_id)}">考頻檢定</button><button class="btn" data-run-search="${esc(p.name)}">全庫搜尋</button>${String(p.type||'').includes('formula')||fb?`<button class="btn" data-formula-action="reverse" data-formula-query="${esc(p.name)}">方劑反查</button><button class="btn" data-formula-action="diff" data-formula-query="${esc(p.name)}">類方鑑別</button><button class="btn" data-formula-action="variants" data-formula-query="${esc(p.name)}">變體／條件式處方</button>`:''}</div></div><div class="card soft entity-display-control"><b>顯示筆數</b><select id="entityDisplayLimit">${entityLimitOptions()}</select><span class="muted">預設 30 筆；資料較多時可改為全部。</span></div><div class="grid entity-section"><div class="card soft"><b>考題位置</b><p>正解 ${esc(qstat.answer||0)}｜干擾 ${esc(qstat.distractor||0)}｜題幹 ${esc(qstat.stem||0)}</p></div><div class="card soft"><b>延伸資料</b><p>證治 ${(ext.zhengzhi_row_ids||[]).length}｜溫病 ${(ext.wenbing_state_row_ids||[]).length}｜source refs ${(p.source_ref_ids||[]).length}</p></div></div>${fb?renderFormulaBlock(fb):''}${renderExtensionRefs(ext,ft)}<div class="split entity-section"><div class="card"><h3>相關考題</h3><div>${entityLimitSummary('顯示',qShown.length,qsample.length)}</div>${qShown.map(renderQuestionFromEvent).join('')||'<p class="muted">無樣本。</p>'}</div><div class="card"><h3>章節來源</h3><div>${entityLimitSummary('顯示',secShown.length,sections.length)}</div>${secShown.map(s=>`<div class="source-row">${esc(s.section_title)}<div class="muted">${esc(s.book_id_y||s.book_id)} p.${esc(s.page_start||'')}</div></div>`).join('')||'<p class="muted">無章節樣本。</p>'}</div></div><div class="split entity-section"><div class="card"><h3>下游關係</h3><div>${entityLimitSummary('顯示',outShown.length,out.length)}</div>${outShown.map(renderRel).join('')||'<p class="muted">無。</p>'}</div><div class="card"><h3>上游關係</h3><div>${entityLimitSummary('顯示',inShown.length,inn.length)}</div>${inShown.map(renderRel).join('')||'<p class="muted">無。</p>'}</div></div>`;
}
...
function renderQuizHome(scope={}){
  ...
  <p class="muted">未套用指定範圍時，預設以「桂枝湯」作為考點關鍵字，並將本次題數控制在 30 題以降低等待感。從知識樹綱目聯動進來時，會直接套用該綱目與下層題目 ID。</p>
  ...
  <div class="quick-quiz-bar">
    ...
    <label>考點<input id="quizFormulaKwQuick" placeholder="例：桂枝湯、小柴胡湯" value="${slabel?'':DEFAULT_QUERY}"></label>
    <label>本次最多題數<input id="quizLimit" type="number" min="1" max="1000" value="30"></label>
  ...
  const defaultFormulaKw=slabel?'':DEFAULT_QUERY;
  if(defaultFormulaKw && !$('#quizFormulaKw').value) $('#quizFormulaKw').value=defaultFormulaKw;
  if($('#quizFormulaKwQuick')) $('#quizFormulaKwQuick').value=$('#quizFormulaKw').value||defaultFormulaKw;
  $('#quizFormulaKwQuick')?.addEventListener('input',()=>{$('#quizFormulaKw').value=$('#quizFormulaKwQuick').value;});
  $('#quizFormulaKw')?.addEventListener('input',()=>{if($('#quizFormulaKwQuick'))$('#quizFormulaKwQuick').value=$('#quizFormulaKw').value;});
...
let relationGraphLoaded=false; async function initRelationGraph(){
  const ensureRendered = (kt)=>{
    ...
    kt.state.centerLimit=Number(kt.state.centerLimit)||80;
...
$('#themeSelect').onchange=e=>document.documentElement.dataset.theme=e.target.value; $$('#navTabs button').forEach(b=>b.onclick=(ev)=>{ev.preventDefault(); ev.stopPropagation(); navigateTab(b.dataset.tab);}); renderGuidePage(); initSearch({autoDefault:false}).catch(e=>{
