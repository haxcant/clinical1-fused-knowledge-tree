...
const titles={
 ...
 quiz:['考題系統','全庫題庫：官方題、Bula 筆記題、seed drill 分層，支援聯動出題。'],
 qhotspot:['考題熱點','正解／干擾／題幹高頻節點，可直接點節點或出題。'],
 pivot:['樞紐分析','讀取 L7 pivot payload，正解／干擾／題幹分開統計。'],
...
const ZH={
  ...
  confidence:{high:'高可信',medium:'中可信',low:'低可信',candidate:'候選',display_or_seed:'顯示／輔助資料',secondary_seed_needs_pdf_verification:'輔助整理，需回 PDF 抽查'},
  status:{display_or_seed:'顯示／輔助資料',usable:'可用',needs_sample_review:'需抽樣確認',secondary_seed_needs_pdf_verification:'輔助整理，需回 PDF 抽查',safe_display:'可安全顯示',review:'待審核'},
...
async function renderGuidePage(){
  ...
  const featureGroups=[
    ...
    {title:'知識結構與圖譜',items:[
      ...
      {tab:'health',name:'資料健康',desc:'查看資料量、模組載入狀態、資料口徑與基本驗收入口。適合檢查是否載入正常。',use:'若某頁無法出題或沒有資料，先到這裡確認資料狀態。'}
    ]},
  ...
  root.innerHTML=`
    <div class="guide-hero card">
      ...
      <div class="guide-quick-actions">
        ...
        <button class="btn" data-jump-tab="health">檢查資料健康</button>
      </div>
    </div>
...
let learningLoaded=false; async function initLearning(){if(learningLoaded&&window.Clinical1LearningInstance)return window.Clinical1LearningInstance; learningLoaded=true; $('#learningRoot').innerHTML='<div class="spinner">載入學習頁題型分析…</div>'; try{window.Clinical1LearningInstance=await Clinical1LearningPage.mount('#learningRoot',{payloadUrl:'data/learning_exam_pattern_payload_v5_0a.json'}); return window.Clinical1LearningInstance;}catch(e){$('#learningRoot').innerHTML=`<div class="card"><h3>學習頁載入失敗</h3><p class="muted">請確認 data/learning_exam_pattern_payload_v5_0a.json 存在，並使用本機伺服器啟動。</p><pre>${esc(e.stack||e.message||e)}</pre></div>`;}}
